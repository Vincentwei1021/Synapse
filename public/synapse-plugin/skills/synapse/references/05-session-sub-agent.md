...
The Synapse Session mechanism tracks **which agent is currently working on which task**. Session data powers the UI observability features: Kanban board worker badges, Task Detail panel active workers, Settings page session list.
...
Main Agent (no session needed):
  Agent ──> synapse_claim_task, synapse_update_task, synapse_report_work (no sessionUuid)
...
- `synapse_session_checkin_task` — before starting work on a task
- `synapse_session_checkout_task` — when done with a task
- Pass `sessionUuid` to `synapse_update_task` and `synapse_report_work` for attribution

**Main agent / Team Lead:** No session tools needed. Call `synapse_update_task` and `synapse_report_work` without `sessionUuid`.
