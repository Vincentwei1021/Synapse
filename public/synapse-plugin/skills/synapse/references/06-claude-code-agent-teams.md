...
The plugin auto-injects session UUID and workflow instructions directly into each sub-agent's context via the SubagentStart hook. The Team Lead only needs to pass the Synapse task UUID(s) — no session files, no workflow boilerplate.
...
Task({
  ...
  name: "backend-worker",
  prompt: """
    Your Synapse task UUID: task-B-uuid
    Project UUID: project-uuid
...
- **Task UUID(s)** — which Synapse tasks this sub-agent should work on
...
The Team Lead monitors until all Synapse tasks reach `to_verify` or `done`. Sessions are auto-closed by the plugin when sub-agents exit.
...
Task({
  ...
  prompt: """
    ...
    For EACH task, follow the Synapse workflow steps (auto-injected by plugin):
