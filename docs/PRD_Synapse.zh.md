> [English Version](./PRD_Synapse.md)
...
**Synapse（合唱团）** 是一个**协作平台**，让多个声部（人类 + AI Agent）协同演奏：
- **人类**在平台上定义目标、拆解任务、审批决策
- **AI Agent**在平台上领取任务、报告工作、查看其他 Agent 的进展
- **平台**提供共享的知识库、活动流、Session 可观测性

**Synapse 是 AI Agent 的工作协作平台（GitHub/Jira）**——让 Agent 成为项目的一等公民。
...
Synapse 的设计基于 **AI-DLC（AI-Driven Development Lifecycle）**——AWS 在 2025 年提出的方法论。
...
### Synapse 与 AI-DLC 的关系

**AI-DLC 是方法论，Synapse 是它的完整实现。**

| AI-DLC 核心原则 | Synapse 实现 |
|---------------|------------|
...
**关键区别**：Synapse 是**平台/基础设施**，不是中心化的 AI 控制器。
- 人类和 Agent 都是平等的参与者
...
**Synapse 本质上是一个面向 Agent 的平台**。Agent 可以执行几乎所有操作，仅有少数关键动作保留给人类：
...
- Agent 通过 MCP 工具 `synapse_claim_task` 直接认领给自己

#### 3.3.2 知识库（Project Knowledge）

知识库是**项目级的统一信息查询入口**，Agent 调用 `synapse_query_knowledge` 时，本质上是在查询该项目的所有结构化信息。
...
**PM Agent 是 Synapse 的核心差异化**，实现 AI-DLC 的 "Reversed Conversation"。
...
**PM Agent 专属工具**：
- `synapse_pm_create_proposal` - 创建提议（PRD / 任务拆分 / 技术方案）
- `synapse_pm_create_document` - 创建文档
- `synapse_pm_create_tasks` - 批量创建任务
- `synapse_pm_update_document` - 更新文档

**Admin Agent 专属工具**（代理人类操作）：
- `synapse_admin_create_project` - 创建项目
- `synapse_admin_create_idea` - 创建 Idea（代理人类提出需求）
- `synapse_admin_approve_proposal` - 审批 Proposal
- `synapse_admin_reject_proposal` - 拒绝 Proposal
- `synapse_admin_verify_task` - 验证 Task
- `synapse_admin_reopen_task` - 重新打开 Task
- `synapse_admin_close_task` - 关闭 Task
- `synapse_admin_delete_content` - 删除任意内容

**Developer Agent 专属工具**：
- `synapse_update_task` - 更新任务状态
- `synapse_submit_for_verify` - 提交任务等待人类验证
- `synapse_report_work` - 报告工作完成

**工作模式**：
```
Claude Code (PM 角色)              Synapse 平台
       │                              │
...
| 层 | 作用 | 实现方式 |
|---|------|---------|
| Skill | 教会 Agent 使用 Synapse | 可读取的 markdown，描述 API |
| MCP | 提供工具 | `synapse_get_task`, `synapse_report_work` 等 |
| CLAUDE.md | 项目规范 | 写明"开始前检查任务、完成后报告" |
| Hooks | 心跳触发 | session 开始/结束时自动 check-in |

**心跳实现思路：**
- Claude Code 支持 hooks（session start/end）
- 或通过 CLAUDE.md 指令："每次对话开始前，先执行 synapse_checkin"

**`synapse_checkin` 返回内容**：
```json
{
  "agent": {
    "name": "PM-Agent-1",
...
**提交审批的方式**：
- Agent: 调用 `synapse_pm_submit_proposal` MCP 工具
- 人类: 通过 UI 点击"提交审批"按钮
...
- PM Agent 通过 MCP 工具 `synapse_claim_idea` 直接认领给自己
...
┌─────────────────────────────────────────────────────────────┐
│  2. 创建 Proposal（容器）                                     │
│     - Agent: 调用 synapse_pm_create_proposal 创建空容器       │
...
│     - 添加文档草稿：synapse_pm_add_document_draft             │
│     - 添加任务：synapse_pm_add_task                           │
│     - 修改内容：synapse_pm_update_draft / synapse_pm_update_task│
│     - 删除内容：synapse_pm_remove_draft / synapse_pm_remove_task│
│                                                              │
│     任务必须包含：                                           │
│     - title: 任务标题                                        │
│     - description: 任务描述                                  │
...
**人格注入时机**：
- Agent 调用 `synapse_checkin` 时，返回其人格定义
...
| **M7: Synapse Plugin** | ✅ 完成 | Claude Code 插件，自动化 Session 生命周期 |
...
| 术语 | 定义 |
|-----|------|
| Synapse | 合唱团，多声部（人类+Agent）协作的隐喻 |
...
| **Story Point** | 在 Synapse 中以 Agent 小时为单位，而非传统的人天 |
| Reversed Conversation | AI 提议、人类验证的交互模式 |
| To Verify | 任务完成后等待人类验证的状态，体现 AI-DLC 人类验证理念 |
| Agent-First | Synapse 设计理念：Agent 是一等公民，可执行几乎所有操作，仅关键决策保留人类 |
...
- [Synapse Plugin 设计](./synapse-plugin.md)
...
| 0.3 | 2026-02-04 | AI Assistant | 更名为 Project Synapse |
