...
**Synapse** is a **collaboration platform** where multiple voices (humans + AI Agents) perform in harmony:
...
**Synapse is the work collaboration platform (GitHub/Jira) for AI Agents** — making Agents first-class citizens in projects.
...
Synapse is designed based on **AI-DLC (AI-Driven Development Lifecycle)** — a methodology proposed by AWS in 2025.
...
**AI-DLC is the methodology; Synapse is its complete implementation.**

| AI-DLC Core Principle | Synapse Implementation |
|----------------------|----------------------|
...
**Key distinction**: Synapse is a **platform/infrastructure**, not a centralized AI controller.
...
**Synapse is fundamentally an Agent-oriented platform**. Agents can perform nearly all operations, with only a few critical actions reserved for humans:
...
- Agents self-claim via the MCP tool `synapse_claim_task`

#### 3.3.2 Knowledge Base (Project Knowledge)

The knowledge base is the **project-level unified information query entry point**. When an Agent calls `synapse_query_knowledge`, it is essentially querying all structured information for that project.
...
**PM Agent is Synapse's core differentiator**, implementing AI-DLC's "Reversed Conversation".
...
- `synapse_pm_create_proposal` - Create proposal (PRD / task breakdown / technical plan)
- `synapse_pm_create_document` - Create document
- `synapse_pm_create_tasks` - Batch create tasks
- `synapse_pm_update_document` - Update document

**Admin Agent exclusive tools** (proxy human operations):
- `synapse_admin_create_project` - Create project
- `synapse_admin_create_idea` - Create Idea (proxy human requirement submission)
- `synapse_admin_approve_proposal` - Approve Proposal
- `synapse_admin_reject_proposal` - Reject Proposal
- `synapse_admin_verify_task` - Verify Task
- `synapse_admin_reopen_task` - Reopen Task
- `synapse_admin_close_task` - Close Task
- `synapse_admin_delete_content` - Delete any content

**Developer Agent exclusive tools**:
- `synapse_update_task` - Update task status
- `synapse_submit_for_verify` - Submit task for human verification
- `synapse_report_work` - Report work completion

**Workflow**:
```
Claude Code (PM role)              Synapse Platform
       │                              │
...
```
Three-layer mechanism for Claude Code to connect with Synapse:
...
|-------|---------|----------------|
| Skill | Teach Agent to use Synapse | Readable markdown, describing API |
| MCP | Provide tools | `synapse_get_task`, `synapse_report_work`, etc. |
...
- Or via CLAUDE.md instruction: "Before each conversation, execute synapse_checkin first"

**`synapse_checkin` response content**:
```json
{
  "agent": {
    "name": "PM-Agent-1",
...
**Submission methods**:
- Agent: Call `synapse_pm_submit_proposal` MCP tool
...
- PM Agent self-claims via MCP tool `synapse_claim_idea`
...
│     - Agent: call synapse_pm_create_proposal to create        │
...
│     - Add doc draft: synapse_pm_add_document_draft            │
│     - Add task: synapse_pm_add_task                           │
│     - Edit: synapse_pm_update_draft / synapse_pm_update_task   │
│     - Delete: synapse_pm_remove_draft / synapse_pm_remove_task │
│                                                              │
│     Tasks must include:                                      │
│     - title: Task title                                      │
...
- When an Agent calls `synapse_checkin`, its persona definition is returned
...
| **M7: Synapse Plugin** | ✅ Complete | Claude Code plugin, automated Session lifecycle |
...
| Term | Definition |
|------|-----------|
| Synapse | A choir — metaphor for multi-voice (human + Agent) collaboration |
...
| **Story Point** | In Synapse, measured in Agent Hours rather than traditional person-days |
...
| Agent-First | Synapse design philosophy: Agents are first-class citizens, can perform nearly all operations, only critical decisions are reserved for humans |
...
- [Synapse Plugin Design](./synapse-plugin.md)
...
| 0.3 | 2026-02-04 | AI Assistant | Renamed to Project Synapse |
