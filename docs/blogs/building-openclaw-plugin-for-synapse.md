# Building an OpenClaw Plugin for Synapse: SSE + MCP to Make AI Agents Truly "Live" in the Workflow
...
This article shares three patterns you can take away directly: **dual-channel architecture** (event push for awareness + tool protocol for execution), **prompt-driven behavior** (message templates guiding autonomous decisions), and **thin proxy bridging** (uniform pattern to expose MCP tools). Using the [Synapse](https://github.com/Synapse-AIDLC/synapse) × [OpenClaw](https://openclaw.ai) integration as a real-world example.

## TL;DR

1. **SSE + MCP as a natural pair** — Synapse's SSE event push (for humans) + MCP tools (for agents), and how they perfectly fit OpenClaw's plugin model
...
[Synapse](https://github.com/Synapse-AIDLC/synapse) implements the AI-DLC workflow:
...
[OpenClaw](https://openclaw.ai) is an AI Agent runtime with plugin support. Our goal: **assign work on Synapse, have the OpenClaw agent automatically detect, claim, execute, and report back**.

Without the plugin, the agent passively waits for terminal input. With it, the workflow becomes:

```
Human assigns task on Synapse UI
        ↓
...
Synapse provides two interfaces for two types of users:

- **MCP tools** — The agent-facing API. 50+ tools covering the full AI-DLC workflow (claim task, create proposal, @mention, elaboration, etc.), authenticated via API Key (`syn_` prefix)
...
Synapse Server
  ...
  │                               Wake agent
  │                                    │
  └── MCP ←── Agent calls ←── "synapse_claim_task + synapse_get_task + start working"
```

No custom API development needed. No changes to Synapse. When Synapse adds a new MCP tool, the plugin adds one line of registration and it just works.

### Reconnection

SSE connections drop due to network hiccups, server restarts, etc. Reconnection uses exponential backoff (1s → 2s → 4s → ... → 30s max). After a successful reconnect, the plugin calls `synapse_get_notifications` to backfill any missed notifications, ensuring zero event loss.
...
|-----------|---------|---------------------|
| `registerService` | Register a background service (with start/stop lifecycle) | **Maintain SSE long-lived connection** — starts on plugin load, continuously listens for Synapse events |
| `registerTool` | Register tools the agent can call | **Expose 40 Synapse MCP tools** — agent can claim tasks, create proposals, etc. |
| `registerCommand` | Register `/command` shortcuts (bypass LLM) | **`/synapse status`** and other quick-query commands |

The combination of these three primitives is the entire plugin skeleton:

```typescript
register(api) {
  // 1. Background service: SSE long-lived connection, listening for Synapse events
  api.registerService({
    id: "synapse-sse",
  ...
  // 2. Tools: 40 Synapse operations exposed to the agent
  ...
  // 3. Commands: /synapse status, /synapse tasks, /synapse ideas
...
This is the heart of the plugin. `registerService` allows a plugin to run a background process, independent of the agent's conversation loop. We use it to maintain the SSE connection to Synapse:
...
await fetch(`${gatewayUrl}/hooks/wake`, {
  ...
  body: JSON.stringify({
    text: "[Synapse] Task assigned: Implement auth module. Task UUID: xxx.",
    mode: "now",
  }),
});
```
...
```
[Synapse] Elaboration answers submitted for idea 'xxx'.
Review the answers with synapse_get_elaboration, then either:
- Call synapse_validate_elaboration with empty issues [] to resolve
...
Every OpenClaw tool is a thin proxy over a Synapse MCP tool. The pattern is highly uniform:
...
|----------|-------|---------------------|
| PM Workflow | 15 | `synapse_claim_idea`, `synapse_create_proposal`, `synapse_pm_create_idea` |
| Developer Workflow | 4 | `synapse_claim_task`, `synapse_report_work`, `synapse_submit_for_verify` |
| Common & Exploration | 20 | `synapse_checkin`, `synapse_list_projects`, `synapse_search_mentionables` |
| Admin | 1 | `synapse_admin_create_project` |
...
This is the easiest trap to fall into. Our npm package name is `@synapse-aidlc/synapse-openclaw-plugin` (with org scope), but the OpenClaw plugin ID **must not include the scope prefix**.

Three places, three different naming rules:

| Location | Value | Notes |
|----------|-------|-------|
| `package.json` → `name` | `@synapse-aidlc/synapse-openclaw-plugin` | npm package name, with org scope |
...
├── package.json              # @synapse-aidlc/synapse-openclaw-plugin
...
Looking back, the most important discovery was that **Synapse's two existing interfaces — SSE for humans and MCP for agents — naturally fit the "real-time awareness + tool operations" plugin model**. No custom API development needed, no changes to Synapse. When Synapse adds a new MCP tool, the plugin adds one line of registration and it just works.
...
Project links:
- **Synapse**: [github.com/Synapse-AIDLC/synapse](https://github.com/Synapse-AIDLC/synapse)
- **OpenClaw Plugin**: [npm @synapse-aidlc/synapse-openclaw-plugin](https://www.npmjs.com/package/@synapse-aidlc/synapse-openclaw-plugin)
