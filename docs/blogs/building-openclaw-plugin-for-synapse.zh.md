# 为 OpenClaw 构建 Synapse 插件：从 SSE 到 MCP，让 AI Agent 真正"活"在工作流里

你有没有遇到过这样的场景：平台上分配了任务，切到终端复制粘贴给 Agent，等它干完再回平台更新状态。Agent 明明很能干，但感知不到"有活来了"，也没法主动汇报 — 它不是团队成员，只是一个你手动喂指令的工具。

这篇文章分享三个可以直接搬走的 pattern：**双通道架构**（事件推送感知 + 工具协议执行）、**Prompt 驱动行为**（消息模板引导自主决策）、**薄代理桥接**（统一模式暴露 MCP 工具）。以 [Synapse](https://github.com/Synapse-AIDLC/synapse) × [OpenClaw](https://openclaw.ai) 的实际集成为例。

## TL;DR

**SSE + MCP 的天然组合** — Synapse 给人类的 SSE 事件推送 + 给 Agent 的 MCP 工具，如何完美适配 OpenClaw 的插件模型<br> 
...
[Synapse](https://github.com/Synapse-AIDLC/synapse) 实现了 AI-DLC 工作流：
...
[OpenClaw](https://openclaw.ai) 是一个 AI Agent 运行时，支持插件扩展。我们的目标是：**在 Synapse 上分配工作，OpenClaw 的 Agent 自动感知、领取、执行、汇报**。

没有插件之前，Agent 只能被动等人在终端里输入指令。有了插件，工作流变成：

```
人类在 Synapse UI 上 assign task
        ↓
...
Synapse 本身为两类用户提供了两种接口：

- **MCP 工具** — 给 AI Agent 用的操作接口，50+ 个工具覆盖完整的 AI-DLC 工作流（claim task、create proposal、@mention、elaboration 等），通过 API Key（`syn_` 前缀）认证
...
Synapse Server
  ...
  │                         唤醒 Agent
  │                              │
  └── MCP ←── Agent 调用 ←── "synapse_claim_task + synapse_get_task + 开始工作"
```

不需要为插件单独开发 API，也不需要改 Synapse 的任何代码。Synapse 未来加新的 MCP 工具时，插件加一行注册就能用。

### 断线重连

SSE 连接会因为网络抖动、服务器重启等原因断开。重连用指数退避（1s → 2s → 4s → ... → 30s max），重连成功后调 `synapse_get_notifications` 补拉断连期间的未读通知，确保不丢事件。
...
| 原语 | 用途 | 在本插件中的角色 |
|------|------|-----------------|
| `registerService` | 注册后台服务（有 start/stop 生命周期） | **维持 SSE 长连接** — 插件加载时启动，持续监听 Synapse 事件 |
| `registerTool` | 注册 Agent 可调用的工具 | **暴露 40 个 Synapse MCP 工具** — Agent 可以 claim task、create proposal 等 |
| `registerCommand` | 注册 `/command` 快捷指令（不经过 LLM） | **`/synapse status`** 等快速查询命令 |

这三个原语的组合，就是插件的全部骨架：

```typescript
register(api) {
  // 1. 后台服务：SSE 长连接，持续监听 Synapse 事件
  api.registerService({
    id: "synapse-sse",
  ...
  // 3. 命令：/synapse status, /synapse tasks, /synapse ideas
...
这是整个插件的核心。`registerService` 允许插件运行一个后台进程，独立于 Agent 的对话循环。我们用它维持和 Synapse 的 SSE 长连接：
...
await fetch(`${gatewayUrl}/hooks/wake`, {
  ...
  body: JSON.stringify({
    text: "[Synapse] Task assigned: 实现用户认证模块. Task UUID: xxx.",
    mode: "now",
  }),
});
```
...
每个 OpenClaw 工具都是对 Synapse MCP 工具的一层薄代理。模式非常统一：
...
| 类别 | 数量 | 典型工具 |
|------|------|---------|
| PM 工作流 | 15 | `synapse_claim_idea`, `synapse_create_proposal`, `synapse_pm_create_idea` |
| Developer 工作流 | 4 | `synapse_claim_task`, `synapse_report_work`, `synapse_submit_for_verify` |
| 通用查询 | 20 | `synapse_checkin`, `synapse_list_projects`, `synapse_search_mentionables` |
...
这是最容易踩的坑。我们的 npm 包名是 `@synapse-aidlc/synapse-openclaw-plugin`（带 org scope），但 OpenClaw 的插件 ID **不能带 scope 前缀**。

三个地方的命名规则各不相同：

| 位置 | 值 | 说明 |
|------|---|------|
| `package.json` → `name` | `@synapse-aidlc/synapse-openclaw-plugin` | npm 包名，带 org scope |
| `openclaw.plugin.json` → `id` | `synapse-openclaw-plugin` | OpenClaw 插件 ID，**不带 scope** |
| `src/index.ts` → `id` | `synapse-openclaw-plugin` | 必须和 manifest 一致 |
...
├── openclaw.plugin.json      # OpenClaw 插件 manifest (id: synapse-openclaw-plugin)
├── src/
...
技术上回头看，最关键的发现是 **Synapse 已有的两套接口 — 给人类的 SSE 和给 Agent 的 MCP — 天然适配了"实时感知 + 工具操作"的插件模型**。不需要为插件专门开发 API，不需要改 Synapse 一行代码。当 Synapse 加一个新 MCP 工具时，插件加一行注册就能用。
...
- **OpenClaw 插件**: [npm @synapse-aidlc/synapse-openclaw-plugin](https://www.npmjs.com/package/@synapse-aidlc/synapse-openclaw-plugin)
