# 为 Claude Code Agent Teams 编写插件：从 Synapse 的实践经验看插件设计模式

> 本文基于 Synapse 项目的实际开发经验，系统介绍 Claude Code 的插件机制，重点探讨如何为 Agent Teams（Swarm 模式）构建插件，以及如何解决多 Agent 协作中的上下文注入难题。
...
2. **以 Synapse 为案例**，展示一个 Agent-first 的任务管理平台是如何通过插件无缝接入 Claude Code 多 Agent 工作流的
...
## 二、Synapse 是什么，解决什么问题

在深入插件实现之前，先简单介绍一下 Synapse。

[Synapse](https://github.com/Synapse-AIDLC/synapse) 是一个 AI Agent 与人类的协作平台，受 [AI-DLC（AI-Driven Development Lifecycle）](https://aws.amazon.com/blogs/devops/ai-driven-development-life-cycle/)方法论启发，实现了其中从 Idea 到 Verify 的核心工作流：
...
在多 Agent 团队场景下，Synapse 需要解决一个具体问题：**可观测性**（Observability）。当 5 个 Sub-Agent 同时在写代码时：
...
Synapse 通过 **Session** 机制追踪这一切——每个工作中的 Agent 拥有一个 Session，Session checkin 到 Task 上，UI 实时展示谁在做什么。
...
这是 Synapse 的核心视图。每个 Task 卡片上的彩色徽章显示当前正在处理该 Task 的 Agent Session。当 Sub-Agent 调用 `synapse_session_checkin_task` 后，徽章实时出现；调用 `checkout` 后消失。Task 在不同列之间的移动（Open → In Progress → To Verify → Done）由 Agent 通过 MCP 工具驱动。

**Task 依赖图（DAG）**

![DAG](../images/dag.png)

Synapse 中的 Task 可以声明依赖关系，形成有向无环图。PM Agent 在创建 Proposal 时通过 `dependsOnDraftUuids` 设置依赖。UI 使用 dagre 自动布局。Team Lead 可以据此决定 spawn 顺序——先处理没有依赖的 Task，被依赖的 Task 完成后，下游 Task 自动解除阻塞。
...
Task 详情页展示了完整的工作历史：哪些 Session 曾经 checkin 过这个 Task，每次 checkin/checkout 的时间，以及 Agent 提交的工作报告。这就是 Synapse 的可观测性——即使 5 个 Agent 同时工作，你也能清楚看到每个人在做什么。

**像素办公室 — Agent 的虚拟工位**

![Pixel Workspace](../images/pixcel-workspace.gif)

这是 Synapse 的趣味功能：每个活跃的 Agent Session 在像素办公室里有自己的工位。Agent checkin 到 Task 时开始"工作"动画，idle 时休息，完成时庆祝。纯粹的可视化娱乐，但能一眼看出团队的工作状态。
...
Task({
  ...
  prompt: """
    你的 Synapse session UUID: ???（Team Lead 还不知道，因为 session 还没创建）
    你的 Synapse task UUID: task-A-uuid

    工作前：
    1. 创建 session: synapse_create_session(...)
    2. Checkin: synapse_session_checkin_task(sessionUuid, taskUuid)
    3. 更新状态: synapse_update_task(taskUuid, "in_progress", sessionUuid)

    工作中：
    4. 汇报进度: synapse_report_work(taskUuid, report, sessionUuid)

    完成后：
    5. Checkout: synapse_session_checkout_task(sessionUuid, taskUuid)
    6. 提交验证: synapse_submit_for_verify(taskUuid, summary)
    7. 关闭 session: synapse_close_session(sessionUuid)
  """
})
```
...
位于 [`.claude-plugin/plugin.json`](https://github.com/Synapse-AIDLC/Synapse/blob/main/public/synapse-plugin/.claude-plugin/plugin.json)，是插件的身份证：
...
插件通过 Marketplace 分发。Marketplace 本质上是一个 JSON 清单文件（[`.claude-plugin/marketplace.json`](https://github.com/Synapse-AIDLC/Synapse/blob/main/.claude-plugin/marketplace.json)），放在 GitHub 公开仓库中即可。Synapse 就是用自己的 GitHub 仓库作为 Marketplace：
...
# 3. 可以指定作用域
/plugin install synapse@synapse-plugins --scope project  # 项目级（团队共享，提交到 git）
/plugin install synapse@synapse-plugins --scope local    # 本地级（仅自己）
```

`source` 字段指向插件在仓库中的相对路径。除了本地路径，还支持指向其他 GitHub repo（`"source": {"source": "github", "repo": "owner/repo"}`）或 Git URL 等来源。

### 4.3 MCP 配置（[.mcp.json](https://github.com/Synapse-AIDLC/Synapse/blob/main/public/synapse-plugin/.mcp.json)）
...
`${SYNAPSE_URL}` 和 `${SYNAPSE_API_KEY}` 是环境变量——Claude Code 在运行时自动替换。用户只需设置环境变量，插件就能连接到正确的服务。

这意味着：**插件安装后，所有 MCP 工具自动可用**。Sub-Agent 也能访问（前提是 MCP 配置在项目级别，而不是用户级别）。

**Synapse 的 MCP 配置**：Synapse 通过 HTTP Streamable Transport 暴露了 50+ 个 MCP 工具，按角色分组（公共工具、PM 工具、Developer 工具、Admin 工具、Session 工具）。用户只需设置两个环境变量 `SYNAPSE_URL` 和 `SYNAPSE_API_KEY`，插件就能连接到 Synapse 服务。API Key 以 `syn_` 前缀开头，携带 Agent 角色信息，服务端据此决定哪些工具可见。
...
**Synapse 的 Skill 体系**：Synapse 包含 [7 个参考文档](https://github.com/Synapse-AIDLC/Synapse/tree/main/public/synapse-plugin/skills/synapse/references)（`references/00` 到 `references/06`），覆盖从公共工具、PM 工作流、Developer 工作流、Admin 工作流，到 Session 管理和 Agent Teams 集成的完整指南。当 Agent 调用 `/synapse` 或 Claude 判断需要 Synapse 知识时，Skill 文档自动加载到上下文中。这相当于给每个 Agent 一本随身携带的操作手册——无论是 Team Lead 还是 Sub-Agent，都能通过 Skill 了解正确的工作流程。
...
配置在 [`hooks/hooks.json`](https://github.com/Synapse-AIDLC/Synapse/blob/main/public/synapse-plugin/hooks/hooks.json)：
...
Synapse 插件的所有 Hook 都使用 `command` 类型——因为 Synapse 的 Hook 逻辑是确定性的（调 API、读写文件、管理状态），不需要 LLM 判断。`prompt` 和 `agent` 更适合需要"理解"代码内容才能做决策的场景，比如在 `Stop` 事件中用 `agent` 类型自动跑测试来判断任务是否真的完成。
...
- **`additionalContext`**：注入到 LLM 的 system context 中——**这是 Hook 影响 Claude 行为的主要手段**。Synapse 的 `SessionStart` Hook 就是通过它把 checkin 结果（身份、任务、通知）注入到 Agent 上下文的
- **`permissionDecision`**：`allow` / `deny` / `ask`，用于 `PreToolUse` 控制工具执行权限
- **`suppressOutput`**：设为 `true` 可静默输出——Synapse 的 `TeammateIdle` Hook 用它来避免每次心跳都弹通知

#### 同步 vs 异步

- **同步 Hook**（默认）：阻塞 Claude 直到完成。适合需要立即影响的场景——Synapse 的 `SubagentStart` 必须同步，因为它要在 Sub-Agent 开始工作前创建好 session 并写入 session file
- **异步 Hook**（`"async": true`）：后台运行，不阻塞。适合不影响流程的场景——Synapse 的 `SubagentStop`（清理资源）和 `TeammateIdle`（心跳）都是异步的

#### 每个 Hook 事件 Synapse 用它做什么

了解了事件、输出格式、同步/异步之后，来看 Synapse 插件如何具体使用每个 Hook。

**`SessionStart` — Checkin + 上下文注入**

这是插件的"开机自检"。注意 `SessionStart` 的 matcher 配置为 `startup|resume|compact`，这意味着它不仅在会话启动和恢复时触发，**还会在上下文压缩（compact）后重新触发**。当长对话触发自动压缩时，之前注入的 Synapse 上下文会随着旧消息被压缩而丢失——`compact` matcher 确保压缩后立即重新注入最新的 checkin 信息，Agent 不会因为上下文压缩而"失忆"。

Synapse 在这里做三件事：

1. 调用 `synapse_checkin()` MCP 工具，获取当前 Agent 的身份（角色、名称、人格）、已分配的 Idea 和 Task、未读通知
2. 将完整的 checkin 结果通过 `additionalContext` 注入到 Claude 的上下文中——Agent 一启动就知道自己是谁、该做什么
3. 扫描 `.synapse/sessions/` 目录，列出已有的 Sub-Agent session 元数据——这是为了处理 Claude Code 会话中断后恢复的情况：上次的 session 文件可能还在，Team Lead 恢复后需要知道哪些 session 仍然存在
...
每次用户输入都会触发，所以必须极快（<100ms）。Synapse 在这里**不做任何网络调用**，只做本地文件检查：

```bash
# on-user-prompt.sh — 纯本地操作，不调 MCP
# 统计 .synapse/sessions/ 下的 json 文件数量
CONTEXT="[Synapse Plugin Active]
...
Synapse 注册了 3 个 `PreToolUse` Hook，分别匹配不同的工具：

| matcher | 脚本 | Synapse 做什么 |
|---------|------|--------------|
| `EnterPlanMode` | [`on-pre-enter-plan.sh`](https://github.com/Synapse-AIDLC/Synapse/blob/main/public/synapse-plugin/bin/on-pre-enter-plan.sh) | 注入 Synapse Proposal 工作流指南——"先创建 Proposal，设置 Task 依赖 DAG，提交审批后再编码" |
| `ExitPlanMode` | [`on-pre-exit-plan.sh`](https://github.com/Synapse-AIDLC/Synapse/blob/main/public/synapse-plugin/bin/on-pre-exit-plan.sh) | 提醒检查——"退出 Plan Mode 前确认 Proposal 已创建并提交" |
| `Task` | [`on-pre-spawn-agent.sh`](https://github.com/Synapse-AIDLC/Synapse/blob/main/public/synapse-plugin/bin/on-pre-spawn-agent.sh) | 捕获 Sub-Agent 的 name/type 写入 pending 文件，供 SubagentStart 认领 |

`EnterPlanMode` 和 `ExitPlanMode` 展示了一个有趣的用法：**用 Hook 引导 Agent 遵循特定的工作流程**。当 Agent 进入 Plan Mode 时，Synapse 自动注入"先创建 Proposal 再编码"的指导；退出 Plan Mode 时，检查是否已有 Proposal。这不是强制阻断（`permissionDecision` 仍然是 `allow`），而是通过 `additionalContext` 做软性引导。

**`SubagentStart` — Session 自动创建 + 直接上下文注入**（核心）

这是 Synapse 插件最核心的 Hook，详见第五章。简要概括：认领 pending 文件 → 创建/复用 Session → 通过 `additionalContext` 将 session UUID + 工作流指令直接注入 Sub-Agent 上下文 → 存储状态映射。Session 文件仅保存最小元数据供其他 Hook 使用。
...
异步 + `suppressOutput: true`。只做一件事：调 `synapse_session_heartbeat` 保持 Session 活跃。Synapse 的 Session 超过 1 小时没有心跳会自动标记为 inactive——这个 Hook 确保只要 Sub-Agent 还在运行，Session 就不会失活。

**`TaskCompleted` — 元数据桥接**

当 Claude Code 内部的 Task 标记完成时，Synapse 检查 task 描述中是否包含 `synapse:task:<uuid>` 标签。如果有，自动执行 `synapse_session_checkout_task`。这是一个优雅的**元数据桥接**模式——通过在 CC Task 描述中嵌入 Synapse task UUID，让两个系统的 Task 生命周期联动。

**`SessionEnd` — 清理 .synapse/ 目录**

会话结束时，检查是否所有 session 文件都已清理、state.json 是否为空。如果是，删除整个 `.synapse/` 目录，不留垃圾文件。

---

## 五、Synapse 插件的完整实现

现在进入正题——Synapse 插件是如何利用上述机制解决多 Agent 协作问题的。
...
Team Lead 调用 Task 工具 spawn Sub-Agent
  ...
  │    写入 .synapse/pending/<name> 文件（捕获 agent name）
  │
  ├─ [SubagentStart] on-subagent-start.sh    ← 核心
  │    认领 pending 文件（原子 mv，处理并发）
  │    创建/复用/重开 Synapse Session（MCP 调用）
  ...
  │    检测 synapse:task:<uuid> 标签，自动 checkout
  │
...
### 5.2 `.synapse/` 目录：连接一切的桥梁

前面多次提到"共享文件系统"，现在展开说。Synapse 插件在项目根目录下维护一个 `.synapse/` 目录（gitignored），它是 Team Lead、Sub-Agent、和所有 Hook 之间的信息枢纽：

```
.synapse/                              # 插件运行时状态（gitignored）
...
Synapse 在 [`synapse-api.sh`](https://github.com/Synapse-AIDLC/Synapse/blob/main/public/synapse-plugin/bin/synapse-api.sh) 中用 `flock` 排他锁解决这个问题：

```bash
# synapse-api.sh 中的 state_set 实现
state_set() {
  local key="$1" value="$2"
  (
    # 获取排他锁，超时 5 秒
...
```
时间线：
  T1  PreToolUse:Task 触发 → 写 .synapse/pending/frontend-worker
  T2  PreToolUse:Task 触发 → 写 .synapse/pending/backend-worker
...
```
SessionStart  → mkdir -p .synapse/（如果不存在）
PreToolUse    → 写 .synapse/pending/<name>
...
SessionEnd    → 如果 sessions/ 为空且 state.json 为空 → rm -rf .synapse/
```
...
当 Sub-Agent 退出时，[`on-subagent-stop.sh`](https://github.com/Synapse-AIDLC/Synapse/blob/main/public/synapse-plugin/bin/on-subagent-stop.sh)（异步执行）负责清理：

1. 查询 Session 的所有活跃 checkin，逐一 checkout
2. 关闭 Synapse Session
...
从 Synapse 插件的实践中，可以提炼出几个通用的设计模式：
...
共享文件系统（`.synapse/` 目录）的价值在于 **Hook 到 Hook** 的状态传递（如 `pending/` 文件从 `PreToolUse` 传递 agent 名称到 `SubagentStart`），但不应该作为 Sub-Agent 上下文注入的主要机制。上下文注入应使用 `SubagentStart` 的 `additionalContext`。
...
Synapse 插件的实践表明，`SubagentStart` 的 `additionalContext`——直接注入 Sub-Agent 上下文——是实现无缝多 Agent 工作流自动化的关键。结合共享文件系统的跨 Hook 状态管理和 `PreToolUse` 的 spawn 时元数据捕获，可以实现完全自动化的 session 生命周期，Team Lead 的 prompt 中零 boilerplate。

如果你对 Synapse 感兴趣，欢迎访问 [GitHub](https://github.com/Synapse-AIDLC/synapse) 了解更多。如果你正在构建自己的 Claude Code 插件，希望本文的经验能帮你少走弯路。
