...
export const adminToolDefinitions = defineOpenClawTools([
  createPassthroughTool<{ name: string; description?: string; groupUuid?: string }>({
    ...
    targetToolName: "synapse_pi_create_research_project",
    mapArgs: ({ name, description, groupUuid }) => {
  ...
  createPassthroughTool<{ name: string; description?: string }>({
    ...
    targetToolName: "synapse_pi_create_project_group",
    mapArgs: ({ name, description }) => {
      const args: Record<string, unknown> = { name };
      if (description) args.description = description;
      return args;
    },
  }),
  createPassthroughTool<{ proposalUuid: string; reviewNote?: string }>({
    ...
    targetToolName: "synapse_pi_approve_experiment_design",
    mapArgs: ({ proposalUuid, reviewNote }) => {
  ...
  createPassthroughTool<{ taskUuid: string }>({
    ...
    targetToolName: "synapse_pi_verify_experiment_run",
    mapArgs: ({ taskUuid }) => ({ runUuid: taskUuid }),
  }),
  createPassthroughTool<{
    taskUuid: string;
    criteria: Array<{ uuid: string; status: string; evidence?: string }>;
  }>({
    ...
    targetToolName: "synapse_mark_acceptance_criteria",
