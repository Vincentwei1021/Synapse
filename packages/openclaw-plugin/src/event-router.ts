...
6. To embed figures, you MUST upload each local image to Synapse via synapse_upload_document_image with experimentUuid "${n.entityUuid}" (or HTTP POST /api/documents/:uuid/images if you already have a documentUuid) and reference it in the Markdown with the returned /api/documents/:uuid/images/... link. Do NOT use catbox, litterbox, imgur, or any other third-party image host — those links expire and are not reliably reachable from end users' browsers.
