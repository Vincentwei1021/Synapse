...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const handleExpiring = () => {
      clientLogger.info("OIDC token expiring, will be auto-renewed");
    };

    // Handle token expired
    const handleExpired = () => {
      clientLogger.info("OIDC token expired");
    ...
    const handleUserLoaded = async (user: User) => {
      clientLogger.info("OIDC user loaded/renewed, syncing token to cookie");
      try {
